...
let peladaConfig = JSON.parse(localStorage.getItem('pelada_config')) || {
    liberada: true,
...
btnGps.addEventListener('click', () => {
    const endereco = encodeURIComponent(inputLocalQuadra.value.trim());
...
btnIniciarCrono.addEventListener('click', () => {
    if (cronometroRodando) {
        ...
        cronometroTimer = setInterval(() => {
            if (tempoSegundos > 0) {
                tempoSegundos--;
                displayCronometro.innerText = formatarTempo(tempoSegundos);
            } else {
                clearInterval(cronometroTimer);
                cronometroRodando = false;
...
btnZerarCrono.addEventListener('click', () => {
    ...
    cronometroRodando = false;
    tempoSegundos = 600;
    displayCronometro.innerText = '10:00';
    btnIniciarCrono.innerText = 'Iniciar';
    btnIniciarCrono.style.backgroundColor = 'var(--verde-confirmado)';
});
...
btnGerarTimes.addEventListener('click', () => {
    ...
    // Ordenamos pela idade decrescente (do mais experiente ao mais jovem)
    jogadoresDeLinha.sort((a, b) => b.idade - a.idade);

    // 3º Passo: Distribuição em zigue-zague (equilibrando idade e posições entre os times)
    let timeIndex = 0;
    let direcao = 1;
    ...
    jogadoresDeLinha.forEach(jogador => {
        ...
        if (timeIndex >= totalTimes) {
            ...
            direcao = -1; // Volta na fila
        } else if (timeIndex < 0) {
            timeIndex = 0;
            direcao = 1; // Avança na fila
        }
    });
    ...
    containerTimes.innerHTML = htmlResultado;
